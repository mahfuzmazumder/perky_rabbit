        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <form class="mt-4" method="POST" action="{{ route('product_type_edit', ['product_type' => $product_type->id]) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="form-group {{ $errors->has('name') ? 'error' : '' }}">
                            ...
                            <input type="text" class="form-control" name="name" value="{{ old('name', $product_type->name) }}"
                        ...
                        <div class="form-group {{ $errors->has('description') ? 'error' : '' }}">
                            <label for="" class="form-label"> @lang('common.product')&nbsp;@lang('common.type_description') <span
                                    class="text-danger">*</span> </label>
                            <textarea class="form-control" name="description" value="{{ old('description', $product_type->description) }}"
                        ...
                        <div class="form-group {{ $errors->has('status') ? 'error' : '' }}">
                            ...
                            <fieldset class="controls">
                                <div class="custom-control custom-radio">
                                    <input type="radio" value="1" {{ old('status', $product_type->status) == '1' ? 'checked' : '' }}
                                        name="status" id="" class="form-check-input">
                                    <label class="form-check-label" for="status"> @lang('common.active') </label>

                                    <input type="radio" value="0" {{ old('status', $product_type->status) == '0' ? 'checked' : '' }}
                                        name="status" id="" class="form-check-input">
